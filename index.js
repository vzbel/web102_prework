...
function addGamesToPage(games) {
  ...
  for (let i = 0; i < games.length; i++) {
    ...
    gameCard.innerHTML = `<img class="game-img" src="${games[i].img}" />
                          <h3>${games[i].name}</h3>
                          <p class="game-card-desc">${games[i].description}</p>
                          <p class="game-card-backers">Backers: ${games[i].backers}</p>`;
...
let moneyPledged = GAMES_JSON.reduce((acc, game) => {
...
let displayStr = `A total of $${moneyPledged.toLocaleString(
  "en-US"
)} has been raised for ${numberOfGames.toLocaleString(
  "en-US"
